...
useHead(() => ({
  title: order.value
    ...
    : t('profile.sellerOrderDetailPage.metaTitle'),
}))

onMounted(async () => {
  try {
    const res = await apiFetch<{ data: ApiOrderDetail }>(`seller-order/${route.params.id}`)
...
async function doAction(endpoint: string, confirmKey: string, body?: Record<string, string>) {
  if (!order.value) return
  if (!confirm(t(confirmKey))) return
  actionLoading.value = true
  try {
    await apiFetch(endpoint, { method: 'PATCH', ...(body ? { body } : {}) })
    const res = await apiFetch<{ data: ApiOrderDetail }>(`seller-order/${order.value.id}`)
...
const canAccept = computed(() => order.value?.status === 'pending_approval')
const canShip = computed(() => order.value?.status === 'pending_shipment')
const canComplete = computed(() => order.value?.status === 'shipped')
const canCancel = computed(() => order.value?.status === 'pending_approval' || order.value?.status === 'pending_payment')
    ...
    <template v-else-if="order">
      ...
      <div class="mb-4 flex items-center justify-between gap-4">
        <SharedNavigationBreadcrumbs
          :show-home="false"
          :items="[
            { label: t('profile.sellerOrderDetailPage.breadcrumb'), to: '/seller/orders' },
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-[380px_1fr]">
        ...
        <!-- Sidebar: buyer info, address, notes, timeline, actions -->
        <div class="space-y-4">

          <!-- Buyer info -->
          <div class="rounded-xl border border-border bg-white p-4">
            <h2 class="mb-3 text-sm font-semibold text-black-normal">
              {{ t('profile.sellerOrderDetailPage.buyer') }}
            </h2>
            <div class="flex items-center gap-3">
              <div class="flex size-10 shrink-0 items-center justify-center rounded-full bg-surface text-sm font-semibold text-navy">
                {{ order.buyer_data.full_name?.[0]?.toUpperCase() }}
              </div>
              <div>
                <p class="text-sm font-medium text-black-normal">{{ order.buyer_data.full_name }}</p>
                <p class="text-xs text-grey-dark-hover" dir="ltr">{{ order.buyer_data.phone }}</p>
              </div>
            </div>
          </div>
          ...
          <!-- Seller actions -->
          <div v-if="canAccept || canShip || canComplete || canCancel" class="space-y-2">
            <BaseButton
              v-if="canAccept"
              variant="primary"
              full-width
              :loading="actionLoading"
              @click="doAction(`seller-order/accept/${order!.id}`, 'profile.sellerOrderDetailPage.actions.acceptConfirm', { type: 'accept_all' })"
            >
              {{ t('profile.sellerOrderDetailPage.actions.accept') }}
            </BaseButton>
            <BaseButton
              v-if="canAccept"
              variant="danger"
              full-width
              :loading="actionLoading"
              @click="doAction(`seller-order/reject/${order!.id}`, 'profile.sellerOrderDetailPage.actions.rejectConfirm')"
            >
              {{ t('profile.sellerOrderDetailPage.actions.reject') }}
            </BaseButton>
            <BaseButton
              v-if="canShip"
              variant="primary"
              full-width
              :loading="actionLoading"
              @click="doAction(`seller-order/make-shipment/${order!.id}`, 'profile.sellerOrderDetailPage.actions.shipConfirm')"
            >
              {{ t('profile.sellerOrderDetailPage.actions.ship') }}
            </BaseButton>
            <BaseButton
              v-if="canComplete"
              variant="primary"
              full-width
              :loading="actionLoading"
              @click="doAction(`seller-order/complete/${order!.id}`, 'profile.sellerOrderDetailPage.actions.completeConfirm')"
            >
              {{ t('profile.sellerOrderDetailPage.actions.complete') }}
            </BaseButton>
            <BaseButton
              v-if="canCancel"
              variant="ghost"
              full-width
              :loading="actionLoading"
              @click="doAction(`seller-order/cancel/${order!.id}`, 'profile.sellerOrderDetailPage.actions.cancelConfirm')"
            >
              {{ t('profile.sellerOrderDetailPage.actions.cancel') }}
            </BaseButton>
          </div>
        </div>

        <!-- Main col: summary + items -->
        <div class="space-y-4">
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  <p class="truncate text-sm font-medium text-black-normal">{{ item.product_name }}</p>
                  ...
                  <p class="mt-0.5 text-xs text-grey-dark-hover">{{ item.status_trans }}</p>
                </div>
                <SharedMoneyAmount
                  :amount="item.price"
                  size="sm"
