...
function buildUrl(page = 1) {
  ...
  return `buyer-order?${params.toString()}`;
}
...
<template>
  <SharedProfileShell role="buyer" @logout="handleLogout">
    ...
    <div class="mb-6 flex items-center justify-between gap-4">
      <h2 class="text-xl font-bold leading-normal text-black-normal">
      ...
      <div class="relative">
        ...
        >
          ...
          {{
            filter === "recent"
              ? t("profile.ordersPage.filters.recent")
              : t("profile.ordersPage.filters.all")
          }}
          <svg
            width="12"
            height="12"
    ...
    <SharedFeedbackEmptyState
      ...
      :title="t('profile.ordersPage.empty.title')"
      :message="t('profile.ordersPage.empty.message')"
      action-to="/"
      ...
      >
        ...
        <div class="flex items-end justify-between gap-3">
          ...
          <div class="flex items-center gap-1.5">
            <!-- Images (max 3) -->
            <div
              class="flex items-center -space-x-2 rtl:space-x-reverse rtl:-space-x-2"
            >
              <img
            ...
            >
              {{
                t("profile.ordersPage.moreProducts", {
                  count: extraCount(order.products_data),
                })
              }}
            </span>
            <!-- Single product name -->
            ...
            <p class="mt-0.5 text-xs text-grey-dark-hover">
              {{
                t("profile.ordersPage.productsCount", {
                  count: order.products_data.length,
                })
              }}
            </p>
          </div>
