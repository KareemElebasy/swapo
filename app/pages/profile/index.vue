...
async function saveInfo() {
  ...
  saveError.value = "";
  try {
    const updated = await apiFetch<{ data: typeof user.value }>(
      "user/profile",
      {
        method: "PATCH",
        body: form,
  ...
  <SharedProfileShell role="buyer" @logout="handleLogout">
    <section class="rounded-sm border border-blue-light bg-white p-6">
          ...
          <div class="mb-6 flex items-center gap-4">
            <div
              class="relative size-20 shrink-0 overflow-hidden rounded-full border-2 border-green bg-surface"
            >
              <img
                v-if="user?.image"
                :src="user.image"
            ...
            <div>
              ...
              <p class="text-sm text-grey-dark-hover">
          ...
          <template v-if="!editing">
            <dl class="divide-y divide-border">
              <div class="flex items-center justify-between py-3">
                ...
                <dt class="text-sm font-medium text-grey-dark-hover">
                  {{ t("profile.page.info.name") }}
                </dt>
              </div>
              <div class="flex items-center justify-between py-3">
                <dd class="text-sm text-black-normal" dir="ltr">
                  {{ user?.email || t("profile.page.info.notSet") }}
                </dd>
                <dt class="text-sm font-medium text-grey-dark-hover">
                  {{ t("profile.page.info.email") }}
                </dt>
              </div>
              <div class="flex items-center justify-between py-3">
                <dd class="text-sm text-black-normal" dir="ltr">
              ...
              <div class="flex items-center justify-between py-3">
                <dd class="text-sm text-black-normal">
                  {{
                    user?.gender === "male"
                      ? t("profile.page.info.male")
                      : user?.gender === "female"
                        ? t("profile.page.info.female")
                        : t("profile.page.info.notSet")
                  }}
                </dd>
                <dt class="text-sm font-medium text-grey-dark-hover">
                  {{ t("profile.page.info.gender") }}
                </dt>
              </div>
              <div class="flex items-center justify-between py-3">
                <dd class="text-sm text-black-normal" dir="ltr">
                  {{ user?.birth_date || t("profile.page.info.notSet") }}
                </dd>
                <dt class="text-sm font-medium text-grey-dark-hover">
                  {{ t("profile.page.info.birthDate") }}
                </dt>
              </div>
            </dl>
          </template>
            ...
            <form class="space-y-4" @submit.prevent="saveInfo">
              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <BaseInput
                  ...
                  :label="
                    t('auth.signupPage.form.nameLabel') +
                    ' – ' +
                    t('auth.signupPage.form.firstNamePlaceholder')
                  "
    ...
    <div
      ...
      class="mt-4 flex items-center justify-between gap-4 rounded-sm border border-green-normal/30 bg-green-light p-5"
    >
      <div class="min-w-0">
        <p class="text-sm font-semibold text-black-normal">
          {{ t("profile.page.becomeSeller.title") }}
        </p>
        <p class="mt-0.5 text-xs text-grey-dark-hover">
          {{ t("profile.page.becomeSeller.description") }}
        </p>
      </div>
      <BaseButton
        variant="primary"
        ...
        class="shrink-0"
      >
        {{ t("profile.page.becomeSeller.cta") }}
      </BaseButton>
    </div>
  </SharedProfileShell>
</template>
