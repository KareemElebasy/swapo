...
function connectSocket(negotiationId: string) {
  ...
  socket.on('connect', async () => {
    // Re-fetch on reconnect to catch missed messages
...
watch(
  ...
  (id) => {
    if (id && negotiationStore.drawerView === 'chat' && negotiationStore.drawerOpen) {
...
watch(
  ...
  (open) => {
    if (!open) {
      socket?.disconnect()
      socket = null
    } else if (negotiation.value?.id && negotiationStore.drawerView === 'chat') {
...
const displayPrice = computed(() => {
  if (negotiation.value?.accepted_offer?.price) return negotiation.value.accepted_offer.price
  if (latestOffer.value?.price) return latestOffer.value.price
  return drawerProduct.value?.price ?? 0
})

const offerSubmitter = computed(() => {
  if (negotiation.value?.accepted_offer) return negotiation.value.accepted_offer.sender
...
const statusBadge = computed(() => {
  ...
  if (neg.status === 'agreed')
    return { text: neg.status_trans, cls: 'bg-status-confirmed-bg text-status-confirmed-text' }

  if (latestOffer.value) {
    const offer = latestOffer.value
    if (offer.status === 'pending')
      return { text: t('negotiation.detail.awaitingReply'), cls: 'bg-status-pending-bg text-status-pending-text' }
    ...
    if (offer.status === 'rejected') {
      ...
      return { text: t('negotiation.offer.rejected'), cls: 'bg-status-canceled-bg text-status-canceled-text' }
    }

    if (offer.status === 'accepted')
      return { text: t('negotiation.offer.accepted'), cls: 'bg-status-confirmed-bg text-status-confirmed-text' }

    if (offer.status === 'cancelled')
      return { text: t('negotiation.offer.cancelled'), cls: 'bg-status-shipped-bg text-status-shipped-text' }
  }

  return { text: neg.status_trans, cls: 'bg-status-pending-bg text-status-pending-text' }
})
...
function computeRemaining() {
  ...
  if (!exp) { remainingTime.value = ''; return }
...
watch(
  ...
  () => {
    if (timerInterval) { clearInterval(timerInterval); timerInterval = null }
    computeRemaining()
    if (remainingTime.value) timerInterval = setInterval(computeRemaining, 30000)
  },
  { immediate: true },
)
...
async function submitOffer() {
  ...
  try {
    if (negotiation.value) {
      await negotiationStore.sendOffer(negotiation.value.id, offerPriceInput.value)
    } else {
      const result = await negotiationStore.startNegotiation(drawerProduct.value.id, offerPriceInput.value)
      if (result?.data?.id) await negotiationStore.fetchNegotiation(result.data.id)
    }
...
async function sendMessage() {
  ...
  sendError.value = ''
  try {
    await negotiationStore.sendMessage(negotiation.value.id, messageText.value.trim())
    messageText.value = ''
  } catch {
...
function formatDay(unixTs: number) {
  return new Date(unixTs * 1000).toLocaleDateString([], { day: 'numeric', month: 'short', year: 'numeric' })
}
...
<template>
  <BaseDrawer
    ...
    side="end"
    :closable="false"
    panel-class="bg-white !w-full sm:!w-[460px] !max-w-full !rounded-s-2xl"
  ...
  >
    ...
    <template #header>
      <!-- Offer form header -->
      <div v-if="isOfferView" class="flex w-full items-center gap-3 border-b border-grey-normal px-4 py-3">
      ...
      <!-- Chat header -->
      <div v-else-if="isChatView" class="flex w-full items-center gap-3 border-b border-grey-normal px-4 py-3">
        ...
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="3" width="18" height="18" rx="2" stroke="currentColor" stroke-width="1.5"/><circle cx="8.5" cy="8.5" r="1.5" fill="currentColor"/><path d="M21 15l-5-5L5 21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
        </div>

        <!-- Product name + seller -->
        <div class="min-w-0 flex-1">
          <p class="truncate text-sm font-semibold text-black-normal">{{ drawerProduct?.product_name }}</p>
          <p class="truncate text-xs text-grey-darker">{{ drawerProduct?.seller_name }}</p>
        </div>
    ...
    <template v-if="isOfferView">
      ...
      <div class="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
        <!-- Product reference -->
        <p class="text-xs text-grey-dark-active">{{ drawerProduct?.product_name }}</p>

        <!-- Original price row -->
        <div class="flex items-center justify-between rounded-xl bg-grey-normal px-4 py-3">
          <span class="text-sm text-grey-darker">{{ t('negotiation.detail.originalPrice') }}</span>
          <SharedMoneyAmount
        ...
        <div>
          ...
          <div class="relative">
            ...
            />
            <span class="absolute inset-s-4 top-1/2 -translate-y-1/2 text-sm font-medium text-grey-darker">ر.س</span>
          </div>
        </div>

        <!-- Shipping info rows -->
        <div class="flex flex-col gap-1.5 rounded-xl bg-grey-normal px-4 py-3 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-grey-dark-active">{{ t('negotiation.detail.shippingNote') }}</span>
          </div>
        </div>

        <p v-if="offerSubmitError" class="text-xs text-status-canceled-text">{{ offerSubmitError }}</p>

        <!-- Policy -->
        <div class="rounded-xl border border-grey-normal px-4 py-3">
          <p class="mb-2 text-xs font-semibold text-black-normal">{{ t('negotiation.startModal.rules.title') }}</p>
      ...
      <!-- Fixed footer -->
      <div class="shrink-0 border-t border-grey-normal px-5 py-4 flex flex-col gap-2">
        <BaseButton
          variant="primary"
          full-width
        ...
        <p class="text-center text-xs text-grey-darker">{{ t('negotiation.startModal.policy') }}</p>
      </div>
    </template>
    ...
    <template v-else-if="isChatView">
      <!-- Loading -->
      <div v-if="negotiationStore.detailLoading" class="flex-1 flex items-center justify-center">
      ...
      <template v-else-if="negotiation">
        ...
        <div class="shrink-0 border-b border-grey-normal bg-white">
          ...
          <div class="flex items-start justify-between px-4 pt-3 pb-2 gap-3">
            ...
            <div class="flex flex-col gap-1">
              <p class="text-xs text-grey-darker">{{ t('negotiation.detail.offerSubmitter') }}</p>
              <div v-if="offerSubmitter" class="flex items-center gap-1.5">
                ...
                >
                  {{ (offerSubmitter.store_name || offerSubmitter.full_name).charAt(0) }}
                </div>
                <span class="text-xs font-medium text-black-normal">
                  {{ isOwn(offerSubmitter.id) ? t('negotiation.detail.you') : (offerSubmitter.store_name || offerSubmitter.full_name) }}
                </span>
              </div>
            </div>
          ...
          <div class="px-4 py-3 text-center">
            <p class="mb-1 text-xs text-grey-darker">{{ t('negotiation.detail.proposedPrice') }}</p>
            <SharedMoneyAmount
              :amount="displayPrice"
              currency="SAR"
              :maximum-fraction-digits="0"
          ...
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="shrink-0 text-grey-darker" aria-hidden="true">
          ...
          <div v-if="isAgreed" class="px-4 pb-3 flex flex-col gap-2">
            <p class="rounded-xl bg-status-pending-bg px-3 py-2 text-xs text-status-pending-text">
        ...
        <div class="flex-1 overflow-y-auto px-4 py-4">
          ...
          >
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-grey-dark" aria-hidden="true">
          ...
          <div v-else class="flex flex-col gap-3">
            <template v-for="group in groupedMessages" :key="group.day">
              ...
              <div class="flex items-center gap-3 my-1">
                ...
                <span class="shrink-0 text-xs text-grey-dark-active">{{ group.day }}</span>
                <div class="flex-1 border-t border-grey-normal" />
              </div>
                ...
                >
                  <div class="mb-2 flex items-center justify-between gap-2">
                    <span
                      ...
                      :class="offerStatusCls[msg.status ?? ''] ?? 'bg-grey-normal text-grey-darker'"
                    >
                      {{ msg.status_trans }}
                    </span>
                    <p class="text-xs text-grey-darker">
                      {{ isOwn(msg.sender.id) ? t('negotiation.detail.you') : (msg.sender.store_name || msg.sender.full_name) }}
                    </p>
                  </div>
                  ...
                  <p class="mt-2 text-end text-[10px] text-grey-dark-active">{{ formatTime(msg.created_at) }}</p>
                </div>
        ...
        >
          <p class="text-sm text-grey-darker">{{ t('negotiation.finished.deliveredMessage') }}</p>
        </div>
      </template>

      <!-- Not found -->
      <div v-else-if="!negotiationStore.detailLoading" class="flex-1 flex items-center justify-center">
        <p class="text-sm text-grey-darker">{{ t('negotiation.errors.loadFailed') }}</p>
      </div>
    </template>
  </BaseDrawer>
</template>
